...
class CrmAuthEventsController extends Controller
{
    public function index(Request $request): View
    {
        abort_unless((bool) config('features.crm_auth_events_log', false), 404);

        $filters = $this->filters($request);

        if (! Schema::hasTable('crm_auth_events')) {
            return view('admin.crm-auth-events', [
                'title' => 'CRM Auth Logs',
                'error' => 'The crm_auth_events table is not available. Run CRM migrations first.',
                'events' => null,
                'filters' => $filters,
            ]);
        }
        ...
        try {
            $query = DB::table('crm_auth_events as e')
            ...
            if (! empty($filters['channel'])) {
                ...
                $query->where('e.meta->channel', $channel);
            }
            ...
            $events = $query->paginate((int) $filters['per_page'])
                ...
                ->through(function ($row) {
                    ...
                    $channel = is_array($meta) && ! empty($meta['channel'])
                        ? ucfirst((string) $meta['channel'])
                        : null;
            ...
            return view('admin.crm-auth-events', [
                'title' => 'CRM Auth Logs',
                'error' => 'Failed to load CRM auth logs.',
                'events' => null,
                'filters' => $filters,
            ]);
        }
    ...
    private function metaSummary(?array $meta): ?string
    {
        ...
        $parts = [];
        foreach (['reason', 'path', 'error', 'channel', 'message'] as $key) {
            if (! empty($meta[$key])) {
                $parts[] = $key.': '.$meta[$key];
            }
        }
